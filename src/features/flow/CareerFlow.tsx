...
import { initialAnswers, STEP_TITLES, type FlowAnswers } from "./types";
...
export function CareerFlow() {
  ...
  const [submitted, setSubmitted] = useState<FlowAnswers | null>(null);
  ...
  const canProceed = useMemo(() => {
    switch (step) {
      case 0:
        return !!answers.basicInfo.name.trim() && !!answers.basicInfo.gender && !!answers.basicInfo.ageRange;
      case 1: {
      ...
      case 3:
        return !!answers.strengthsInterests.primaryStrength && !!answers.strengthsInterests.energizingTasks;
      case 4:
  ...
  const handleSubmit = () => {
    ...
    console.log("Career flow submission:", answers);
  };
...
// ---------- Review ----------
function ReviewStep({ answers, onJump }: { answers: FlowAnswers; onJump: (step: number) => void }) {
...
// ---------- Success ----------
function SuccessView({ data }: { data: FlowAnswers }) {
  return (
