...
type SetAnswers = (updater: (prev: FlowAnswers) => FlowAnswers) => void;
...
export function Step1BasicInfo({
  ...
  const set = (patch: Partial<FlowAnswers["basicInfo"]>) =>
    setAnswers((p) => ({ ...p, basicInfo: { ...p.basicInfo, ...patch } }));
...
export function Step2Education({
  ...
  const set = (patch: Partial<FlowAnswers["education"]>) =>
    setAnswers((p) => ({ ...p, education: { ...p.education, ...patch } }));

  const droppedOut = education.status === "dropped-school" || education.status === "dropped-college";
...
export function Step3Work({
  ...
  const set = (patch: Partial<FlowAnswers["workExperience"]>) =>
...
// ---------- Page 4 ----------
export function Step4Strengths({
  answers,
  setAnswers,
}: {
  answers: FlowAnswers;
  ...
  const set = (patch: Partial<FlowAnswers["strengthsInterests"]>) =>
    setAnswers((p) => ({
      ...p,
...
// ---------- Page 5 ----------
export function Step5Satisfaction({
  answers,
  setAnswers,
}: {
  answers: FlowAnswers;
  ...
  const set = (patch: Partial<FlowAnswers["satisfaction"]>) =>
...
// ---------- Page 6 ----------
export function Step6Preferred({
  answers,
  setAnswers,
}: {
  answers: FlowAnswers;
  ...
  const set = (patch: Partial<FlowAnswers["preferredRole"]>) =>
...
export function Step7Cognitive({
  answers,
  setAnswers,
}: {
  answers: FlowAnswers;
  ...
  const otherActive = selections.includes("other") || !!otherValue;

  const toggle = (v: string) => {
    setAnswers((p) => {
      const cur = p.cognitiveProfile.selections;
      const next = cur.includes(v) ? cur.filter((x) => x !== v) : [...cur.filter((x) => x !== "prefer-not" || v === "prefer-not"), v];
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-accent/40 bg-accent/20 px-5 py-4 text-sm text-accent-foreground">
        This section is completely optional. Understanding how your mind naturally works can help
        us suggest environments where you'll genuinely thrive — not just perform.
      </div>
...
// ---------- Page 8 ----------
export function Step8Character({
  answers,
  setAnswers,
}: {
  answers: FlowAnswers;
  ...
  const set = (v: string) =>
    setAnswers((p) => ({ ...p, coreCharacter: { openResponse: v } }));
